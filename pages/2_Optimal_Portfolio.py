...
with dataset:
    ...
    rf = np.mean(riskFreeData["3-months T-Bill"])
...
with results:
    ...
    df = pd.concat([stdList, annualReturnList], axis=1)
...
with simulation:
    ...
    for i in iterList:
        ...
        portfolios = pd.DataFrame({'Asset 1': assetList1, 'Asset 2': assetList2, 'Weight 1': portfolioWeightList1, 'Weight 2': portfolioWeightList2, 'Return 1': portfolioReturnList1,
    ...
    with optimalPortfolio:
        ...
        optimal = pd.DataFrame(weights, columns=["w1", "w2"], index=iterList)
        optimal['expected return'] = returns
        optimal['volatility'] = volatilities

        st.subheader("Optimal Portfolios based on your selection")
        ...
        st.subheader("Optimal Portfolios after the intelligent elimination")
        st.table(optimal)
        ...
        optimalData = pd.DataFrame(np.array(portfolioWeights), columns = ['Weight 1', 'Weight 2', 'Weight 3'], index = index)

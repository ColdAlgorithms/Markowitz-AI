...
with userInput:
    ...
    defaultBegDate = datetime(datetime.now(
    ).year-1, datetime.now().month, datetime.now().day)
    # Calender components for user's date entry
    startDate = firstCol.date_input(
        "Pick a start date ", value=defaultBegDate, max_value=datetime.now())
    endDate = secCol.date_input(
        "Pick an end date", min_value=startDate, max_value=datetime.now())
...
with dataset:
    ...
    try:
        for stockOption in choosenTickerList:
            historicalData[stockOption] = get_data(
    ...
    # Market Data
    historicalData["SP500"] = get_data(
        "^GSPC", start_date=startDate, end_date=endDate)['adjclose']

    riskFreeData["3-months T-Bill"] = get_data(
...
with simulation:
    ...
    with optimalPortfolio:
        ...
        try:
            optimalData = DataFrame(np.array(portfolioWeights), columns = ['Weight 1', 'Weight 2', 'Weight 3'])
